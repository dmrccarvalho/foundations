...
object ForLoopExercises {
  ...
  def sum(numbers: List[Int]): Int =
    foldLeft(numbers, 0)(_ + _) // Same as below
  ...
  def min(numbers: List[Int]): Option[Int] =
    foldLeft(numbers, Option.empty[Int])((state, number) =>
      state match {
        ...
        case Some(value) => Some(number min value)
      }
    )
  /*{
    ...
    for (number <- numbers)
      state match {
        ...
        case Some(value) => state = Some(number min value)
      }
  ...
  def wordCount(words: List[String]): Map[String, Int] =
    foldLeft(words, Map.empty[String, Int])((frequencies, word) =>
      frequencies.get(word) match {
        case Some(frequency) => frequencies.updated(word, frequency + 1)
        case None            => frequencies.updated(word, 1)
      }
    )
